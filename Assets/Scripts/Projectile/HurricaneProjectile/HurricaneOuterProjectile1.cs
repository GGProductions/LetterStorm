...
public class HurricaneOuterProjectile1 : MonoBehaviour {
	...
	void Update () {
        ...
        transform.Translate(Vector3.back * moveY);
        if (transform.position.z > 26.4f || transform.position.z < -27f || transform.position.x < -20f || transform.position.x > 20f) { Destroy(this.gameObject); }
