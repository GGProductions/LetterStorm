...
public class Boss_move_PtP : MonoBehaviour {
    ...
    private Vector3 midScreenPosition;
    private float tem; //variable to hold deltatime for moving woth cos function
    ...
    /// <summary>
    /// setting up listener to acknowldge that a wrong letter has hi the boss (this message comes from Boss_3d_WordGen
    /// </summary>
    void OnEnable()
    {
    ...
    void Awake()
    {
        ...
        midScreenPosition = new Vector3(0f, 0f, 4);
        gameObject.transform.position = midScreenPosition;
        tem = 0;
  ...
  //  public float smooth = 5.0F;
    ...
    IEnumerator NotStart()
    {
        ...
        while (true)
        {
            yield return StartCoroutine(MoveObject(transform, point_topleft, point_mid, 3.0f));
            yield return StartCoroutine(MoveObject(transform, point_mid, point_toright, 3.0f));
            yield return StartCoroutine(MoveObject(transform, point_toright, point_mid, 3.0f));
            yield return StartCoroutine(MoveObject(transform, point_mid, point_topleft, 3.0f));
        }
    }

    void Start() { StartCoroutine(NotStart()); }
/*
    IEnumerator Start()
    {
        //Vector3 pointA_init = new Vector3(transform.position.x, transform.position.y, transform.position.z);
